...
if(isset($_POST['submit'])){
 ...
 // Check if $medicine_name is already an array
if (is_array($medicine_name)) {
  // If it's an array, directly implode it
  $medicines = implode(",", $medicine_name);
} else {
  // If it's not an array, convert it to an array and then implode
  $medicines = implode(",", (array)$medicine_name);

  }
  $quantity = $_POST['qty'];

// Check if $quantity is already an array
if (is_array($quantity)) {
    // If it's an array, directly implode it
    $qty = implode(",", $quantity);
} else {
    // If it's not an array, handle it accordingly (e.g., assign it as is or convert to an array)
    $qty = $quantity;
}
